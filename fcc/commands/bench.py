...
from fcc.utils.utils import apps_to_update_menu, check_bench_for_updates, deploy_new_bench, get_bench_menu, get_bench_updated_apps, get_team_with_menu,TOKEN_AUTH_HEADER, humanify, validate_bench, validate_team 
...
def get_benches(team):
    ...
    if benches_req.ok:
        try:
            ...
            benches_table = Table("Bench Title","Status","Number of Sites","Total Apps","Version","Bench Name","Created",title=f"All Benches of {team}")
            for bench in all_benches["message"]:
                benches_table.add_row(bench["title"],bench["status"],str(bench["number_of_sites"]),str(bench["number_of_apps"]),bench["version"],bench["name"],humanify(bench["creation"]))
